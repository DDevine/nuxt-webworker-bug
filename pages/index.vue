...
export default {
  ...
  watch: {
    workers (workers) {
      if (workers.length === 0) {
        this.notification = 'Zero free Web Workers - click "Create more Workers"'
      }
    }
  },
  methods: {
    test () {
      ...
      if (worker) {
        ...
        this.notification = 'No more test workers available'
      }
    },
    long (milliseconds) {
    ...
    removeWorker () {
      ...
      if (this.longRunningWorkers.includes(worker) > -1) {
        this.longRunningWorkers.splice(this.longRunningWorkers.indexOf(worker), 1)
      }
